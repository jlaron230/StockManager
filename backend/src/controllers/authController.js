...
// Connexion utilisateur : vérifie l'email, prépare la session si rôle valide
async function loginUser(req, res, next) {
  ...
  try {
    ...
    if (user[0] != null) {
      ...
      // Vérifie que l'utilisateur a un rôle valide
      if (firstUser.role && (firstUser.role === "admin" || firstUser.role === "employe" || firstUser.role === "responsable")) {
        ...
        req.user = firstUser;
        next(); // Continue vers le middleware suivant (ex: vérification du mot de passe)
      } else {
...
// Vérifie si la session est active et renvoie les infos utilisateur
...
// Déconnexion : détruit la session et supprime le cookie
...
// Envoi un mail avec un lien de réinitialisation de mot de passe
...
// Réinitialise le mot de passe avec un nouveau mot de passe via token
async function resetPassword(req, res) {
  ...
  try {
    ...
    // Met à jour le mot de passe et supprime les tokens
    await client.query(
        `UPDATE users
