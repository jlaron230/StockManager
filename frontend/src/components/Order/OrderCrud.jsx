...
const OrderCrud = () => {
    ...
    // Vérification de session / rôle admin
    useEffect(() => {
    ...
    // Chargement des providers et produits (une fois)
    useEffect(() => {
        if (!isAdmin) return;

        axios
            .get(`${import.meta.env.VITE_BACKEND_URL}/providers`, { withCredentials: true })
        ...
        <div className="max-w-3xl mx-auto p-4">
            ...
            <Formik
                ...
                onSubmit={async (values, { resetForm }) => {
                    ...
                    if (!window.confirm(`Valider cette commande pour un montant total de ${total.toFixed(2)} € ?`))
                        return;
            ...
            >
                {({ values, setFieldValue }) => {
                    // Met à jour l'id_user dès que user est dispo
                    useEffect(() => {
                        if (user) setFieldValue("id_user", user);
                    }, [user, setFieldValue]);
                    ...
                    // Identifie les fournisseurs sélectionnés
                    const selectedProductIds = values.products.map((p) => Number(p.id_product));
                    const selectedProducts = products.filter((prod) =>
                        selectedProductIds.includes(prod.id_product)
                    );
                        ...
                        <Form className="space-y-6">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {values.products.map((p, index) => (
                                            <div key={index} className="flex gap-4 items-end">
                                                ...
                                                <ButtonOrder buttonType="button" onClick={() => remove(index)} ButtonName="✕" />
                                            </div>
                                        ))}
                                    </div>
                                )}
                            ...
                            {values.products.length > 0 && uniqueProviderIds.length === 1 && (
                                <p className="text-green-600 text-xl">
                                    ...
                                    {providers.find((p) => p.id_provider === uniqueProviderIds[0])?.nom}
                                </p>
                            )}
